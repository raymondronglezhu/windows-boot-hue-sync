...
def log(message: str) -> None:
    ...
    with LOG_PATH.open("a", encoding="utf-8") as handle:
        handle.write(f"[{timestamp}] [startup] {message}\n")
...
def main() -> int:
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        log(f"Startup attempt {attempt}/{MAX_ATTEMPTS}")
        result = run_hue("activate-scene", SCENE_NAME, "--room", ROOM_NAME)
        if result.returncode == 0:
            log("Startup scene activation succeeded")
            return 0

        if attempt < MAX_ATTEMPTS:
            time.sleep(RETRY_DELAY_SECONDS)

    log("Startup scene activation failed after all attempts")
    return 1
